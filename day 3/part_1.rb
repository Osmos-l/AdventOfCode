module Event
    ...
    def self.event_description()
        ...
        puts "(solution by https://github.com/delventhalz/advent-of-code-2023/blob/main/03-gear-ratios/1.js)"
        puts
    end
    ...
    def self.isSymbol(char)
        return char != "." && char.to_i.to_s != char.to_s
    end
        ...
        forEachEngine(engine_schematic) do |char, char_cords|
            if isSymbol(char)
                forEachSurrounding(engine_schematic, char_cords) do |surrounder, surrounder_cords|
                    sum = sum + extractNumber(engine_schematic, surrounder_cords) if isNumber(surrounder)
                end
            end
        end
        puts "Sum: #{sum}"
